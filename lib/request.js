...
const requestInternal = (request, resolve) => {
    const { type = 'base', method = 'GET', uri, version = 'HTTP/1.1', headers: _headers = EMPTY_OBJECT, body, info, timeout = 30000, connectTimeout = timeout, dataTimeout = timeout, follow = false, certificate = false, download = true, downloadLimit = Infinity, correctHeaders = true, rejectUnauthorized = true, ...rest } = request
